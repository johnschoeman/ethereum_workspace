...
contract('Dead', (accounts) => {
  describe('Function: heartbeat', () => {
    ...
    it('should increment lastHeartbeat by heartbeatPeriod', async () => {
      ...
      const heartbeatPeriod = await dead.heartbeatPeriod.call();  // call will never change state, i.e. won't cause an actual bc transaction
      const timeToHeartbeat = heartbeatPeriod.dividedToIntegerBy('2');

      const initialLastHeartbeat = await dead.lastHeartbeat.call();

      await utils.increaseTime(timeToHeartbeat.toNumber());  //beware js is bad with big numbers. use big number library.
